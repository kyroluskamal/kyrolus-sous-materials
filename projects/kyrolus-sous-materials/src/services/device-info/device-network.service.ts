...
export class DeviceNetworkService {
  ...
  ? toSignal(
      merge(
        ...
        startWith(null),
        map(() => (typeof navigator !== 'undefined' && 'onLine' in navigator
...
function defaultNetInfo(): NetInfo {
  return { effectiveType: null, saveData: null, downlink: null, rtt: null };
}

function readConnection(conn: any): NetInfo {
  // effectiveType narrowing إلى literal union معروف أو 'unknown'
  let effective: EffectiveConnectionType | null = null;
